...
export default {
    ...
    methods: {
        ...
        registrarTexto() {
            ...
            if (this.nombre == null || this.enlace == null || this.correlacion == null) {
        ...
        editarTexto() {
            this.submitted = true;
            //validar si hay campos vacios
            if (this.datosArreglo.nombre == null || this.datosArreglo.enlace == null || this.datosArreglo.correlacion == null) {
                // si alguno de los campos esta vacio, no enviar el formulario y mostrar un mensaje de error
                this.$toast.add({
                    severity: "error",
                    summary: "Error",
            ...
            formData.append('enlace', this.datosArreglo.enlace);
            formData.append('correlacion', this.datosArreglo.correlacion);
    ...
    data() {
        return {
            ...
            nombre: null,
            enlace: null,
            correlacion: null, // Agrega esta línea para el correlacion
            searchQuery: '',
...
<template>
    ...
    <div class="cards-container">
        <Card v-for="datosTexto in filteredBanner" style="width: 40em; margin-bottom: 40px;">
            <template #title> Titulo: {{ datosTexto.nombre }}</template>
            <template #subtitle> Contenido: {{ datosTexto.enlace }}
            <br>Identificador: {{ datosTexto.correlacion }} </template>
        ...
        <div class="field">
            <form @submit.prevent="registrarTexto">
                ...
                <div class="field col-12 md:col-12">
                    ...
                    <Textarea inputId="minmax" v-model="nombre" :min="0" :max="10000" :showButtons="true" />
                </div>

                <div class="field col-12 md:col-12">
                    <label for="minmax">{{ this.Subtitulo }}</label>
                    <Textarea inputId="minmax" v-model="enlace" :min="0" :max="10000" :showButtons="true" />
                </div>

                <div class="field col-12 md:col-12">
                    <label for="minmax"> Identificador </label>
                    <InputText inputId="minmax" v-model="correlacion" />
                </div>
    ...
    <Dialog v-model:visible="editarDialog" :breakpoits="{ '960px': '75vw', '640px': '85vw' }" :style="{ width: '25vw' }"
        ...
        <div class="field">
            <form @submit.prevent="editarTexto">
                ...
                <div class="field col-12 md:col-12">
                    <label for="minmax">{{ this.Titulo }}</label>
                    <Textarea inputId="minmax" v-model="datosArreglo.nombre" :min="0" :max="10000" :showButtons="true" />
                </div>

                <div class="field col-12 md:col-12">
                    <label for="minmax">{{ this.Subtitulo }}</label>
                    <Textarea inputId="minmax" v-model="datosArreglo.enlace" :min="0" :max="10000" :showButtons="true" />
                </div>

                <div class="field col-12 md:col-12">
                    <label for="minmax"> Identificador </label>
                    <Textarea inputId="minmax" v-model="datosArreglo.correlacion" :min="0" :max="10000" :showButtons="true" />
                </div>
...
.card {
    ...
    /* Esto permite que cada card tome el espacio necesario y se expanda según el enlace */
    margin: 10px;
